...
t_dis <- 4 # Disinfection strategy ([0= No dis], [1=7am], [2=3pm], [3=11pm], [4=12 and 6pm])
...
df_stl_btn <- as.data.frame(cbind(Csp, GC_Inf, Vs, FSA,  n_stl, TEsh_stl, dt, TEhm, k, prev))
...
corRes <- data.frame(type = c("TEhm", "Csp", "Vs", "dt","n", "TEsh", "GC_inf", "prev", "k"),
...
+ theme_minimal() + theme(axis.text.x = element_text(angle = 10))
