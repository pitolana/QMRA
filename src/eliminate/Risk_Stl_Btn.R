...
t_btw_push <- runif(simNum, 1, 10) # Time between pushing the buttons (assumed)
...
c_t_btw  <- cor.test(df_stl_btn$risk_0.50, df_stl_btn$t_btw_push,method = "spearman", exact=F)  
...
c_GC_inf <- cor.test(df_stl_btn$risk_0.50, df_stl_btn$GC_TCID50, method = "spearman", exact=F) 
...
corRes <- data.frame(type = c("TEhm", "Csp", "Vs", "t_btw","n", "TEsh", "GC_inf", "prev", "k"),
                     rho = c(c_TEhm$estimate, c_Csp$estimate, c_Vs$estimate, c_t_btw$estimate, c_n$estimate, 
                             c_TEsh$estimate, c_GC_inf$estimate, c_prev$estimate, c_k$estimate))
